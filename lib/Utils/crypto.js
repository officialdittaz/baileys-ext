...
const { curve, keyhelper, crypto: libCrypto } = libsignal;
...
export const Curve = {
    ...
    sharedKey: (privateKey, publicKey) => {
        const shared = curve.calculateAgreement(generateSignalPubKey(
    ...
    verify: (pubKey, message, signature) => {
        try {
            curve.verifySignature(generateSignalPubKey(pubKey), message,
                signature);
            return true;
        } catch {
            return false;
...
export function aesEncryptGCM(plaintext, key, iv, additionalData) {
    ...
    return Buffer.concat([cipher.update(plaintext), cipher.final(), cipher
        .getAuthTag()
    ]);
}
...
export function aesEncryptWithIV(buffer, key, IV) {
    const aes = createCipheriv("aes-256-cbc", key, IV);
    return Buffer.concat([aes.update(buffer), aes.final()]);
}
...
export async function hkdf(buffer, expandedLength, info) {
    const inputKeyMaterial = new Uint8Array(buffer instanceof Uint8Array ?
        buffer : new Uint8Array(buffer));
    const salt = info.salt ? new Uint8Array(info.salt) : new Uint8Array(0);
    const infoBytes = info.info ? new TextEncoder().encode(info.info) :
        new Uint8Array(0);
    
    const importedKey = await subtle.importKey("raw",
        inputKeyMaterial, { name: "HKDF" }, false, ["deriveBits"]);
...
export async function derivePairingCodeKey(pairingCode, salt) {
    ...
    const saltBuffer = new Uint8Array(salt instanceof Uint8Array ? salt :
        new Uint8Array(salt));
    
    const keyMaterial = await subtle.importKey("raw",
        pairingCodeBuffer, { name: "PBKDF2" }, false, ["deriveBits"]);
    const derivedBits = await subtle.deriveBits({
            name: "PBKDF2",
            salt: saltBuffer,
            iterations: 2 << 16,
