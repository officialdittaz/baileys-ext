...
export const generateWAMessageContent = async (message, options) => {
    ...
    // Handle modern interactive buttons (BYPASS - NEW METHOD)
        ...
        // Convert buttons to interactive format
        ...
        if (image) {
            const preparedMedia = await prepareWAMessageMedia({ image: image }, options);
            ...
            const preparedMedia = await prepareWAMessageMedia({ video: video }, options);
            messageContent.header = {
                title: title || "",
                hasMediaAttachment: hasMediaAttachment || true,
            ...
            const preparedMedia = await prepareWAMessageMedia(docInput, options);
            messageContent.header = {
                title: title || "",
                hasMediaAttachment: hasMediaAttachment || true,
            ...
            messageContent.header = {
                ...
                productMessage: {
                    ...
                    businessOwnerJid:
                        businessOwnerJid || product.businessOwnerJid || options.userJid,
                },
            };
        } else if (title) {
            messageContent.header = {
        ...
        // Create interactive message wrapped in viewOnceMessage
        m = {
            viewOnceMessage: {
                message: {
        ...
        if ("text" in message) {
            ...
            if ("caption" in message) {
                ...
                interactiveMessage.header = {
                    title: message.title,
                    subtitle: message.subtitle,
                    hasMediaAttachment: message.hasMediaAttachment
...
export const generateWAMessageFromContent = (jid, message, options) => {
    ...
    if (quoted && !isJidNewsletter(jid)) {
        ...
        } else if (innerMessage[key]) {
            innerMessage[key].contextInfo = contextInfo;
        }
    }

    // Check if it's an interactive message for additional nodes
    const isInteractive =
    ...
    const msg = WAProto.WebMessageInfo.fromObject(messageJSON);

    // Add additional nodes for interactive messages
    if (isInteractive) {
 ...
 */
export function getAggregateResponsesInEventMessage({ eventResponses }, meLid) {
