...
export const prepareWAMessageMedia = async (message, options) => {
    ...
    if (isNewsletter) {
        ...
        try {
            if (requiresThumbnailComputation) {
                ...
                if (!uploadData.width && originalImageDimensions) {
                    ...
                    logger?.debug("set dimensions for newsletter media");
                }
            }
            if (requiresDurationComputation) {
                uploadData.seconds = await getAudioDuration(filePath);
                logger?.debug("computed audio duration for newsletter");
            }
            if (requiresWaveformProcessing) {
                uploadData.waveform = await getAudioWaveform(filePath, logger);
                logger?.debug("processed waveform for newsletter");
            }
        } catch (error) {
            logger?.warn({ trace: error.stack }, "failed to obtain newsletter media metadata");
        }
        
        const fileSha256B64 = fileSha256.toString("base64");
        const { mediaUrl, directPath } = await options.upload(filePath, {
            fileSha256B64: fileSha256B64, // Fixed: use fileSha256B64 for raw media
            mediaType: mediaType,
        ...
        if (cacheableKey) {
            logger?.debug({ cacheableKey }, "set cache for newsletter media");
