...
const storeMappingFromEnvelope = async (stanza, sender, repository,
    decryptionJid, logger) => {
    ...
    if (senderAlt && isLidUser(senderAlt) && isPnUser(sender) &&
        ...
        try {
            ...
            logger.debug({ sender, senderAlt },
                "Stored LID mapping from envelope");
        } catch (error) {
            logger.warn({ sender, senderAlt, error },
...
export const extractAddressingContext = (stanza) => {
    ...
    const addressingMode = stanza.attrs.addressing_mode || (sender
        ?.endsWith("lid") ? "lid" : "pn");
    if (addressingMode === "lid") {
        // Message is LID-addressed: sender is LID, extract corresponding PN
        // without device data
        senderAlt =
            stanza.attrs.participant_pn || stanza.attrs.sender_pn || stanza
...
export function decodeMessageNode(stanza, meId, meLid) {
    ...
    if (isPnUser(from) || isLidUser(from) || isHostedLidUser(from) ||
        isHostedPnUser(from)) {
        if (recipient && !isJidMetaAI(recipient)) {
            if (!isMe(from) && !isMeLid(from)) {
                throw new Boom(
                "receipient present, but msg not from me", { data: stanza });
            }
            if (isMe(from) || isMeLid(from)) {
                fromMe = true;
            }
    ...
    const key = {
        remoteJid: chatId,
        remoteJidAlt: !isJidGroup(chatId) ? addressingContext.senderAlt :
            undefined,
        fromMe,
        id: msgId,
        participant,
        participantAlt: isJidGroup(chatId) ? addressingContext.senderAlt :
            undefined,
    ...
    if (addressingContext.addressingMode === 'lid' && !fromMe && key
        .remoteJidAlt) {
        ...
        async decrypt() {
            ...
            if (Array.isArray(stanza.content)) {
                for (const { tag, attrs, content } of stanza.content) {
                    if (tag === "verified_name" &&
                        content instanceof Uint8Array) {
                        const cert = proto.VerifiedNameCertificate
                            .decode(content);
                        const details = proto.VerifiedNameCertificate
                            .Details.decode(cert.details);
                        fullMessage.verifiedBizName = details
                            .verifiedName;
                    }
                    if (tag === "unavailable" && attrs.type ===
                        "view_once") {
                        fullMessage.key.isViewOnce =
                        true; // TODO: remove from here and add a STUB TYPE
                    }
                    ...
                    decryptables += 1;
                    let msgBuffer;
                    const decryptionJid = await getDecryptionJid(author,
                        repository);
                    if (tag !== "plaintext") {
                    ...
                    try {
                        const e2eType = tag === "plaintext" ?
                            "plaintext" : attrs.type;
                        switch (e2eType) {
                            case "skmsg":
                            ...
                            default:
                                throw new Error(
                                    `Unknown e2e type: ${e2eType}`);
                        }
                        let msg = proto.Message.decode(
                            e2eType !== "plaintext" ?
                            unpadRandomMax16(msgBuffer) : msgBuffer
                        );
                        ...
                        if (msg.senderKeyDistributionMessage) {
                            ...
                            try {
                                ...
                                logger.error({ key: fullMessage.key,
                                        err },
                        ...
                        const errorContext = {
                            key: fullMessage.key,
                            err,
                            messageType: tag === "plaintext" ?
                                "plaintext" : attrs.type,
                            sender,
                            author,
                            isSessionRecordError: isSessionRecordError(
                                err),
                        };
                        logger.error(errorContext,
                            "failed to decrypt message");
                        fullMessage.messageStubType = proto
                            .WebMessageInfo.StubType.CIPHERTEXT;
            ...
            if (!decryptables && !fullMessage.key?.isViewOnce) {
                fullMessage.messageStubType = proto.WebMessageInfo
                    .StubType.CIPHERTEXT;
