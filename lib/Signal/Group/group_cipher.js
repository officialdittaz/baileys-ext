...
export class GroupCipher {
    ...
    async encrypt(paddedPlaintext) {
        const record = await this.senderKeyStore.loadSenderKey(this
            .senderKeyName);
        if (!record) {
        ...
        await this.senderKeyStore.storeSenderKey(this.senderKeyName,
            record);
    ...
    async decrypt(senderKeyMessageBytes) {
        ...
        const senderKeyState = record.getSenderKeyState(senderKeyMessage
            .getKeyId());
        if (!senderKeyState) {
    ...
    getSenderKey(senderKeyState, iteration) {
        ...
        if (senderChainKey.getIteration() > iteration) {
            if (senderKeyState.hasSenderMessageKey(iteration)) {
                const messageKey = senderKeyState.removeSenderMessageKey(
                    iteration);
                if (!messageKey) throw new Error(
        ...
        while (senderChainKey.getIteration() < iteration) {
            senderKeyState.addSenderMessageKey(senderChainKey
                .getSenderMessageKey());
    ...
    async getCipherText(iv, key, plaintext) {
        try {
            return crypto.encrypt(key, plaintext, iv);
        } catch (e) {
            throw new Error("InvalidMessageException");
        }
    }
