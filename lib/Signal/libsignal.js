...
export function makeLibSignalRepository(auth, logger, pnToLIDFunc) {
    ...
    const repository = {
        ...
        async injectE2ESession({ jid, session }) {
            ...
            return parsedKeys.transaction(async () => await cipher.initOutgoing(session), jid);
        },
        ...
        async validateSession(jid) {
            try {
                ...
                return { exists: true };
            } catch { return { exists: false, reason: "validation error" }; }
        },
        async deleteSession(jids) {
            ...
            return parsedKeys.transaction(async () => await auth.keys.set({ session: sessionUpdates }), `delete-${jids.length}-sessions`);
        },
        async migrateSession(fromJid, toJid) {
            ...
            logger.debug({ fromJid, totalDevices: userDevices.length, devicesWithSessions: deviceJids.length, devices: deviceJids }, "bulk device migration complete");
